...
//        }
        ...
        {
            get { return _sessionModel ?? (_sessionModel = new SessionModel(base.HttpContext)); }
        ...
        [HttpPost]
        public ActionResult Login(string documentName, string bookmark, LogOnModel loginModel)
        {
            SessionModel.Clear();
            if (_login.CanLogin(loginModel.UserName, loginModel.Password))
            {
                ...
                return redirect(documentName, bookmark, loginModel.UserName);
            }
        ...
        private RedirectResult redirect(string documentName, string bookmark, string username)
        {
            return Redirect(
                    _urlGeneration.GetUrlToRedirect(
                        username, documentName, bookmark
                        ));
        }
    }
}
