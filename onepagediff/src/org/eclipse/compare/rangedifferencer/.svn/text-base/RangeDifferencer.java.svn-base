 ...
 * <code>IRangeComparator</code>s. <p> To use the differencer, clients provide
 * an <code>IRangeComparator</code> that breaks their input data into a sequence
 * of comparable entities. The differencer returns the differences among these
 * sequences as an array of <code>RangeDifference</code> objects
 * (<code>findDifferences</code> methods). Every <code>RangeDifference</code>
 ...
 * sides. <p> Alternatively, the <code>findRanges</code> methods not only return
 * objects for the differing ranges but for non-differing ranges too. <p> The
 * algorithm used is an objectified version of one described in: <it>A File
...
public final class RangeDifferencer {
  ...
  public static RangeDifference[] oldDifferencer(IRangeComparator left,
    IRangeComparator right) {
    return OldDifferencer.findDifferences((IProgressMonitor)null, left, right);
  }

  public static RangeDifference[] newDifferencer(LCSSettings settings,
    IRangeComparator left, IRangeComparator right) {
    return RangeComparatorLCS.findDifferences((IProgressMonitor)null, settings,
      left, right);
  }
  ...
  public static RangeDifference[] findDifferences(IRangeComparator left,
    IRangeComparator right) {
    return findDifferences((IProgressMonitor)null, new LCSSettings(), left,
      right);
  }
   ...
   * differences are returned as a list of <code>RangeDifference</code>s. If no
   * differences are detected an empty list is returned. If the ancestor range
   * comparator is <code>null</code>, a two-way comparison is performed.
   * 
   ...
   */
  public static RangeDifference[] findDifferences(LCSSettings settings,
    IRangeComparator ancestor, IRangeComparator left, IRangeComparator right) {
   ...
   */
  public static RangeDifference[] findDifferences(IProgressMonitor pm,
    LCSSettings settings, IRangeComparator ancestor, IRangeComparator left,
    IRangeComparator right) {
    try {
      if (ancestor == null)
      ...
      if (rightAncestorScript != null) {
        ...
        leftAncestorScript = findDifferences(monitor.newChild(50), settings,
          ancestor, left);
      }
      ...
      while (myIter.fDifference != null || yourIter.fDifference != null) {
        ...
        diff3.add(createRangeDifference3(myIter, yourIter, diff3, right, left,
   ...
   * to <code>findDifferences</code>, the result contains
   ...
   * <code>RangeDifference</code> elements for non-differing ranges too. If the
   * ancestor range comparator is <code>null</code>, a two-way comparison is
   * performed.
   * 
   * @param ancestor the ancestor range comparator or <code>null</code>
   * @param left the left range comparator
  ...
  public static RangeDifference[] findRanges(IProgressMonitor pm,
    ...
    for (int i = 0; i < in.length; i++) {
      ...
      rd = new RangeDifference(RangeDifference.NOCHANGE, mstart, es
        .rightStart()
        - mstart, ystart, es.leftStart() - ystart, astart, es.ancestorStart()
        - astart);
      if (rd.maxLength() > 0)
        out.add(rd);
    ...
    rd = new RangeDifference(RangeDifference.NOCHANGE, mstart, right
      .getRangeCount()
      - mstart, ystart, left.getRangeCount() - ystart, astart, ancestor
      .getRangeCount()
      - astart);
    if (rd.maxLength() > 0)
      out.add(rd);
  ...
  private static RangeDifference createRangeDifference3(
    DifferencesIterator myIter, DifferencesIterator yourIter, List diff3,
    IRangeComparator right, IRangeComparator left, int changeRangeStart,
    int changeRangeEnd) {
    ...
    if (myIter.getCount() == 0) { // only left changed
      ...
      RangeDifference l = (RangeDifference)myIter.fRange.get(myIter.fRange
        .size() - 1);
    ...
    if (yourIter.getCount() == 0) { // only right changed
      ...
      RangeDifference l = (RangeDifference)yourIter.fRange.get(yourIter.fRange
        .size() - 1);
    ...
    return new RangeDifference(kind, rightStart, rightEnd - rightStart,
      leftStart, leftEnd - leftStart, changeRangeStart, changeRangeEnd
        - changeRangeStart);
  }

  /*
   * Tests whether <code>right</code> and <code>left</code> changed in the same
   * way
   */
