...
public class TextNodeComparator
  implements IRangeComparator, Iterable<TextNode> {
  ...
  public void markAsDeleted(int start, int end, TextNodeComparator oldComp,
    ...
    while (deletedNodes.size() > 0) {
      ...
      if (nextLeaf != null) {
        nextResult = nextLeaf.getLastCommonParent(deletedNodes.get(deletedNodes
          .size() - 1));
      }
      else {
      ...
      if (prevResult.getLastCommonParentDepth() == nextResult
        ...
        if (deletedNodes.get(0).getParent() == deletedNodes.get(
          ...
          // now THIS is tricky
          double distancePrev = deletedNodes.get(0).getParent().getMatchRatio(
            prevResult.getLastCommonParent());
          double distanceNext = deletedNodes.get(deletedNodes.size() - 1)
            .getParent().getMatchRatio(nextResult.getLastCommonParent());
      ...
      if (prevResult.getLastCommonParentDepth() > nextResult
        ...
        if (prevResult.isSplittingNeeded()) {
          prevLeaf.getParent().splitUntill(prevResult.getLastCommonParent(),
            prevLeaf, true);
        }
  ...
  public void expandWhiteSpace(boolean expand) {
    getBodyNode().expandWhiteSpace(expand);
  }
