...
public class HTMLDiffer {
  ...
  private boolean expandWhitespace = false;
  ...
  public void diff(TextNodeComparator leftComparator,
    ...
    RangeDifference[] differences = new RangeDifference[0];
    LCSSettings settings = new LCSSettings();
    settings.setUseGreedyMethod(false);
    differences = RangeDifferencer.newDifferencer(settings, leftComparator,
      rightComparator);

    List<RangeDifference> pdifferences = preProcess(differences);
    numDifferences = pdifferences.size();
    ...
    for (RangeDifference d : pdifferences) {
      ...
      if (d.leftStart() > currentIndexLeft) {
        rightComparator.handlePossibleChangedPart(currentIndexLeft, d
          .leftStart(), currentIndexRight, d.rightStart(), leftComparator);
      }
      if (d.leftLength() > 0) {
    ...
    if (currentIndexLeft < leftComparator.getRangeCount()) {
      rightComparator.handlePossibleChangedPart(currentIndexLeft,
        leftComparator.getRangeCount(), currentIndexRight, rightComparator
          .getRangeCount(), leftComparator);
    }

    rightComparator.expandWhiteSpace(expandWhitespace);
  ...
  private List<RangeDifference> preProcess(RangeDifference[] differences) {
    ...
    for (int i = 0; i < differences.length; i++) {
      ...
      while (i + 1 < differences.length
        ...
        && score(leftLength, differences[i + 1].leftLength(), rightLength,
          differences[i + 1].rightLength()) > (differences[i + 1].leftStart() - leftEnd)) {
  ...
  public boolean isExpandWhitespace() {
    return expandWhitespace;
  }

  public void setExpandWhitespace(boolean expandWhitespace) {
    this.expandWhitespace = expandWhitespace;
  }
